 ...
 * 
 * 
 * Create Socket and connect to server  in line 285
 * 
 */
...
public class Client extends JFrame implements Runnable {
	...
	public Client() {
		...
		clientTitle.setFont(new Font("Lucida Grande", Font.PLAIN, 15));
		clientTitle.setBounds(5, 4, 58, 16);
		panel.add(clientTitle);
		clientTitle.setBackground(Color.LIGHT_GRAY);
				...
				if(clientCounter<1) {
					...
					if(!rdbtnAes.isSelected() && !rdbtnDes.isSelected()) {
						...
						if(cryptedPane.getText().isEmpty()) {
							...
							pw.println(Client.name+">"+cryptedPane.getText());
							Server.msgText.append(cryptedPane.getText()+"\n");
							Server.msgText.append(Client.name+" > "+textPane.getText()+"\n");
							textPane.setText("");
							cryptedPane.setText("");
						}
					}
		...
		Server.msgText.setBounds(6, 6, 469, 314);
		panel_4.add(Server.msgText);
		
		try{
			InetAddress ip= InetAddress.getLocalHost();
			s=new Socket(ip,12001);
